            ...
            if not retrieved_docs:
                ...
                if st.session_state.conversation_memory:
                    ...
                    for conv in recent_conversations:
                        # conv가 리스트인 경우 문자열로 변환
                        if isinstance(conv, list):
                            conv_str = " ".join(str(item) for item in conv)
                        else:
                            conv_str = str(conv)
                        conversation_context += f"{conv_str}\n"
                    ...
                    for chunk in stream:
                        ...
                        if hasattr(chunk, 'content'):
                            ...
                            if isinstance(chunk_content, list):
                                chunk_text = "".join(str(item) for item in chunk_content)
